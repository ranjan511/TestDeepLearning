...
import org.nd4j.common.primitives.Pair; //changed by Sukanta
...
public class DataFrames {
	...
	//SQLContext sc =  data.sqlContext();
        JavaRDD<Row> rows = data.map(new ToRow(schema));
        return spark.createDataFrame(rows, fromSchema(schema));
    }
    ...
    public static Dataset<Row> toDataFrameSequence(Schema schema, JavaRDD<List<List<Writable>>> data) {
        ...
        return spark.createDataFrame(rows, fromSchemaSequence(schema));
    }
